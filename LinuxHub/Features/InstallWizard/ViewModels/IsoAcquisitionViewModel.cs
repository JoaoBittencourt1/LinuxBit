...
namespace LinuxHub.Features.InstallWizard.ViewModels
{
    ...
    public class IsoAcquisitionViewModel : ObservableObject
    {
        ...
        public void SelectManualIso(string path)
        {
            if (!IsValidIso(path))
            {
                Notify?.Invoke("Erro", "ISO inválida ou muito pequena.\nSelecione uma ISO Linux válida.", true);
                ManualIsoPath = null;
        ...
        private async Task DownloadAsync()
        {
            ...
            try
            {
                ...
                ResolvedIsoPath = path;
                OnPropertyChanged(nameof(DisplayedDistro));
                Notify?.Invoke("LinuxHub", $"Download concluído: {path}", false);
            }
            catch (OperationCanceledException)
            {
                Notify?.Invoke("LinuxHub", "Download cancelado.", false);
            }
            catch (Exception ex)
            {
                Notify?.Invoke("Erro no download", ex.Message, true);
            }
            finally
            {
                IsDownloading = false;
